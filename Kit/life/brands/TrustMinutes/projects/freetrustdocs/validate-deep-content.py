...
DD = f'{P}/declaration-deep-content'
...
spec48 = json.load(open(f'{DD}/INPUT-SPEC-48.json'))
spec3 = json.load(open(f'{DD}/INPUT-SPEC.json'))
spec = {**spec48, **spec3}  # union: 48 batch states + original UT/CA/TX spec
research = json.load(open(f'{P}/all-states-research.json'))
rs = {s['state']: s for s in research['states']}

files = sorted(glob.glob(f'{DD}/declaration-deep-*.json'))
rows, fails = [], 0
for f in files:
    name = os.path.basename(f)
    if name == 'declaration-deep-texas.json' and False:
        pass
    slug = name.replace('declaration-deep-', '').replace('.json', '')
    try:
        d = json.load(open(f))
